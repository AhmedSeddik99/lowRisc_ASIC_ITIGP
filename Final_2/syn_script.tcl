...
set RTL "./RTL/ibex_wrapper.sv"
set RTL_PATH "./RTL/"
set CONSTRAINTS "./constraints/zero-ris5y_constraints.tcl"
set SYN_CONST "./constraints/syn_const.tcl"
set DONT_USE_CONST "./constraints/dont_use_const_hendawi.tcl"
set WORK "./work"
set LOG "./log"
set REPORT "./reports"
set OUTPUT "./output"
...
foreach i $pkg {
	analyze -library work -format sverilog "$RTL_PATH$i" >> "$LOG/analyze_package.log"
}

foreach i $hier_0 {
	analyze -library work -format sverilog "$RTL_PATH$i" >> "$LOG/analyze_hier_0.log"
}

foreach i $hier_1 {
	analyze -library work -format sverilog "$RTL_PATH$i" >> "$LOG/analyze_hier_1.log"
}

foreach i $hier_2 {
	analyze -library work -format sverilog "$RTL_PATH$i" >> "$LOG/analyze_hier_2.log"
}

foreach i $hier_3 {
	analyze -library work -format sverilog "$RTL_PATH$i" >> "$LOG/analyze_hier_3.log"
}

analyze -library work -format sverilog "$RTL_PATH/ibex_top.sv" > "$LOG/analyze_top.log"

analyze -library work -format sverilog "$RTL_PATH$DESIGN.sv" > "$LOG/analyze_wrapper.log"
...
source $CONSTRAINTS > "$LOG/source_constraints.log"
#return
source $SYN_CONST > "$LOG/source_syn_const.log"
#return
...
#remove_unconnected_port [get_cells ibex_top] -blast_buses > "$LOG/remove_unconnected_port.log"

compile_ultra -retime > "$LOG/compile_ultra.log"
...
report_timing -delay_type max -max_paths 500 > "$REPORT/syn_setup.rpt"
report_timing -delay_type min -max_paths 500 > "$REPORT/syn_hold.rpt"
