...
`ifndef SYNTHESIS
      ...
      `ASSERT(SboxPermutationCheck_A, &sbox_perm_test, clk_i, rst_ni)
`endif
...
`ifdef FPV_ON
      ...
      int shift;
      int unsigned sk, sj;
      `ASSUME(SjSkRange_M, (sj < NumSboxes) && (sk < NumSboxes), clk_i, rst_ni)
      `ASSUME(SjSkDifferent_M, sj != sk, clk_i, rst_ni)
      `ASSUME(SjSkStable_M, ##1 $stable(sj) && $stable(sk) && $stable(shift), clk_i, rst_ni)
      `ASSERT(SboxInputIndexGroupIsUnique_A,
          !((((sbox_in_indices[sj * 4 + 0] + shift) % LfsrDw) == sbox_in_indices[sk * 4 + 0]) &&
            ...
            (((sbox_in_indices[sj * 4 + 3] + shift) % LfsrDw) == sbox_in_indices[sk * 4 + 3])), clk_i, rst_ni)
      ...
      int y;
      int unsigned ik;
      `ASSUME(IkYRange_M, (ik < LfsrDw) && (y == 1 || y == -1), clk_i, rst_ni)
      `ASSUME(IkStable_M, ##1 $stable(ik) && $stable(y), clk_i, rst_ni)
      `ASSERT(IndicesNotAdjacent_A, (sbox_in_indices[ik] - sbox_in_indices[(ik + y) % LfsrDw]) != 1, clk_i, rst_ni)
`endif
...
`ifndef SYNTHESIS
  ...
  `ASSERT(NextStateCheck_A, ##1 lfsr_en_i && !seed_en_i |=> lfsr_q ==
      compute_next_state(coeffs, $past(entropy_i), $past(lfsr_q)), clk_i, rst_ni)
...
`endif
  ...
  //`ASSERT(NoLockups_A, (!entropy_i) && (!seed_en_i) |=> !lockup, clk_i, !rst_ni, clk_i, rst_ni)
  ...
  if (ExtSeedSVA) begin : gen_ext_seed_sva
    ...
    `ASSERT(ExtDefaultSeedInputCheck_A, (seed_en_i && rst_ni) |=> lfsr_q == $past(seed_i), clk_i, rst_ni)
  end
  ...
  if (LockupSVA) begin : gen_lockup_mechanism_sva
    ...
    `ASSERT(LfsrLockupCheck_A, lfsr_en_i && lockup && !seed_en_i |=> !lockup, clk_i, rst_ni)
  end
...
`ifndef SYNTHESIS
    ...
    `ASSERT(MaximalLengthCheck0_A, cnt_q == 0 |-> lfsr_q == DefaultSeedLocal,
        clk_i, !rst_ni || perturbed_q, clk_i, rst_ni)
