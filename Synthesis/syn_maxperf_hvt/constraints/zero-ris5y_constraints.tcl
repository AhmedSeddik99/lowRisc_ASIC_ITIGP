...
create_generated_clock -master_clock $CLK1_NAME -source [get_ports clk_i] \
                       -name "cluster_clock_gating" [get_port ibex_top_inst/core_clock_gate_i/clk_o] \
                       -divide_by 1
...
foreach input $synth_inputs {
    ...
    set delay [expr [lindex $input 1]/100] 
    set_input_delay $delay -clock [get_clocks "cluster_clock_gating"] [get_ports $port]
}

foreach output $synth_outputs {
    set port [lindex $output 0] 
    set delay [expr [lindex $output 1]/100] 
    set_output_delay $delay -clock [get_clocks "cluster_clock_gating"] [get_ports $port]
}
